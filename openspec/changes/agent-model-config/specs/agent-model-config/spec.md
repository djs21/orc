...
When `[models]` config is set for a role, the agent file SHALL additionally include a `model` field after `mode` and before `permission` in the YAML front-matter.
