...
case "$action" in
  ...
  nav-project)
    ...
    target="$(_orc_tmux list-windows -t "$SESSION" -F '#{window_name}' 2>/dev/null \
      | grep -v '/' | grep -v '^orc$' | grep -v '^status$' | grep -v 'board' | grep -v '^_' | head -1 || true)"
    if [[ -n "$target" ]]; then
      _orc_tmux select-window -t "${SESSION}:${target}"
    else
      _orc_tmux display-message "No project window found"
    fi
    ;;
  ...
  nav-board)
    project="${1:-}"
    if [[ -n "$project" ]]; then
      _orc_tmux select-window -t "${SESSION}:${project}/board" 2>/dev/null \
        || _orc_tmux display-message "Board window not found for ${project}"
    fi
    ;;
  ...
  nav-goal-pane)
    ...
    if [[ -n "$window" ]]; then
      ...
      if [[ -n "$goal_pane" ]]; then
        _orc_tmux select-pane -t "$(_tmux_target "$window" "$goal_pane")"
      else
        # Fallback: select first pane
        _orc_tmux select-pane -t "$(_tmux_target "$window").0" 2>/dev/null || true
      fi
    fi
    ;;
  ...
  orch-send)
    ...
    if [[ -z "$window" || -z "$orc_id_pattern" || -z "$slash_cmd" ]]; then
      _orc_tmux display-message "Menu action error: missing arguments"
      exit 1
    fi
  ...
  *)
    _orc_tmux display-message "Unknown menu action: ${action}"
    ;;
esac
